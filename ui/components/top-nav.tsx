...
export function TopNav({ showCreateNow = false, onContinueAsGuest }: TopNavProps) {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/40 backdrop-blur-xl border-b border-white/5 shadow-sm">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <Link href="/" className="text-white font-semibold tracking-widest text-sm hover:text-white/80 transition-colors">
            RESERV
          </Link>
            ...
            {authUser ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-slate-900 border-white/10 text-white min-w-[180px]">
                  <DropdownMenuItem disabled className="text-white/60 cursor-default">
                    Profile
                  </DropdownMenuItem>
